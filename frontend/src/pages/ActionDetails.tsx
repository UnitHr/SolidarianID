      ...
      const fetchActionDetails = async () => {
        try {
          const action = await fetch(
            `http://localhost:3000/api/v1/actions/${actionId}`
          );
          if (!action.ok) {
          ...
          const cause = await fetch(
            `http://localhost:3000/api/v1/causes/${data.causeId}`
          );
          if (!cause.ok) {
            ...
            <Col xs={12} md={9}>
              ...
              <p className="text-center text-muted mt-3">
                <OverlayTrigger
                  placement="top"
                  overlay={
                    <Tooltip id={`tooltip-cause-${action?.causeId}`}>
                      See cause details
                    </Tooltip>
                  }
                >
          ...
          <Row>
            <Col>
              <p>
                ...
                <span className="text-capitalize">{action?.status}</span>
              </p>
              <p>
                <strong>Type:</strong>{" "}
